...
Button = types.KeyboardButton('بازی جدید')
markup.add(Button)
...
def send_age(message):
    user_age = bot.send_message(
        message.chat.id, 'تاریخ تولدت رو قشنگ به فرمی که می گم بنویس : درست بنویسی هااا \n   نحوه درست نوشتن :         روز   /   ماه   /   سال  \n/Exit تو هر مرحله برای خروج \n')
    bot.register_next_step_handler(user_age, salshomar)


@bot.message_handler(commands=['voice'])
def send_voice(message):
    user_txt = bot.send_message(
        message.chat.id, ' یه چیز انگلیسی تایپ کن که برات ویسش رو بفرستم\nاگر هم می خوای خارج بشی کامند زیر رو بزن\n\n/Exit')
...
def send_argmax(message):
    list_number = bot.send_message(
        message.chat.id, ' می خوام بهت بگم عدد چندمیه از همه بزرگتره \n چند تا عدد به فرمی که میگم بنویس مثلا \n 12,13,14,2,1,... \n\nاگر هم می خوای خارج بشی کامند زیر رو بزن  \n\n/Exit')
    bot.register_next_step_handler(list_number, find_index)


@bot.message_handler(commands=['qrcode'])
def send_qrcode(message):
    user_qr = bot.send_message(
        message.chat.id, 'یه چیزی بنویس که برات کیو ار کدش رو  بفرستم \n\nاگر هم می خوای خارج بشی کامند زیر رو بزن \n\n/Exit')
    ...
    if message.text != '/Exit':
        try:
            if message.text == 'بازی جدید':
                ...
                bot.send_message(message.chat.id, 'یک عدد بین صفر تا صد که فکر میکنی جوابه حدس بزن :', reply_markup=markup)
                bot.register_next_step_handler_by_chat_id(message.chat.id, guess_number)
            elif int(message.text) < number_rand:
                user_guess = bot.send_message(message.chat.id, 'برو بالاتررر', reply_markup=markup)
                bot.register_next_step_handler(user_guess, guess_number)
            elif int(message.text) > number_rand:
